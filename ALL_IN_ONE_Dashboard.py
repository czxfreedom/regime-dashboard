...
if st.button("🔄 Refresh All Tabs", key="global_refresh"):
    st.cache_data.clear()
...
tab_functions = {
    "Cumulative pnl matrix": render_cumulative_pnl,
    "Daily pnl and trades matrix 30 mins": render_pnl_and_trades,
    "Spread matrix": render_spread_analysis,
    "Volatility matrix": render_vol_and_hurst,
    "Hurst matrix": render_macro_view
}

for tab, name in zip(tabs, tab_names):
    with tab:
        st.markdown(f"### {name}")
        if st.button(f"↻ Refresh {name}", key=f"refresh_{name}"):
            st.cache_data.clear()
            st.experimental_rerun()
        tab_functions[name]()
