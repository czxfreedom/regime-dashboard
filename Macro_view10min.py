# Save this as pages/06_Daily_Hurst_Table_10min.py in your Streamlit app folder
...
st.set_page_config(
    page_title="Daily Hurst Table (10min)",
    page_icon="📊",
    layout="wide"
)
...
st.title("Daily Hurst Table (10min)")
st.subheader("All Trading Pairs - Last 24 Hours (All Intervals)")

# Define parameters for the 10-minute timeframe
timeframe = "10min"
lookback_days = 1  # 24 hours
rolling_window = 60  # Window size for Hurst calculation (10 hours of 10min data)
expected_points = 144  # Expected data points per pair over 24 hours (24 * 6)
...
with col2:
    ...
    day_selection = st.selectbox(
        "Time Period",
        ["Last 24 Hours", "Today (UTC)", "Yesterday (UTC)"]
    )
...
def generate_sample_price_data(token, lookback_days):
    ...
    # Generate timesteps (10-minute intervals)
    timesteps = pd.date_range(start=start_time, end=end_time, freq='10T')
...
# Determine time period based on user selection
def get_time_range(selection):
    now = datetime.utcnow()
    
    if selection == "Last 24 Hours":
        # Previous 24 hours from now
        end_time = now
        start_time = end_time - timedelta(days=1)
    elif selection == "Today (UTC)":
        # Today in UTC (00:00:00 to now)
        end_time = now
        start_time = datetime(now.year, now.month, now.day, 0, 0, 0)
    else:  # "Yesterday (UTC)"
        # Yesterday in UTC (00:00:00 to 23:59:59)
    ...
    calc_start = start_time - timedelta(days=1)
    
    return calc_start, start_time, end_time

# Get time ranges
calc_start, display_start, display_end = get_time_range(day_selection)

# Fetch and calculate Hurst for a token with 10min timeframe
def fetch_and_calculate_hurst(token):
    if db_available:
        try:
            ...
            WHERE created_at BETWEEN '{calc_start}' AND '{display_end}'
            AND pair_name = '{token}';
            """
        ...
        df = generate_sample_price_data(token, (display_end - calc_start).days)
    ...
    try:
        ...
        # Resample to 10-minute timeframe
        ohlc = df['final_price'].resample(timeframe).ohlc().dropna()
        
        # Calculate Hurst with rolling window (60 bars for 10min data)
        ohlc['Hurst'] = ohlc['close'].rolling(rolling_window).apply(universal_hurst)
        
        # Filter to display period
        display_data = ohlc[(ohlc.index >= display_start) & (ohlc.index <= display_end)]
        
        # Add regime classification
        display_data['regime_info'] = display_data['Hurst'].apply(detailed_regime_classification)
        display_data['regime'] = display_data['regime_info'].apply(lambda x: x[0])
        display_data['regime_desc'] = display_data['regime_info'].apply(lambda x: x[2])
        
        # Create time labels in HH:MM format
        display_data['time_label'] = display_data.index.strftime('%H:%M')
...
# Create table for display with all 10-minute intervals
if token_results:
    ...
    # Display the table
    time_range_str = f"{display_start.strftime('%Y-%m-%d %H:%M')} to {display_end.strftime('%Y-%m-%d %H:%M')} UTC"
    st.markdown(f"## Hurst Exponent Table (10min timeframe)")
    st.markdown(f"### Time Period: {time_range_str}")
    ...
    # Create table height based on number of rows
    table_height = min(700, 100 + (len(all_times) * 35))  # Adjust height based on row count
    
    st.dataframe(styled_table, height=table_height, use_container_width=True)
...
with st.expander("Understanding the 10-Minute Hurst Table"):
    st.markdown("""
    ### How to Read This Table
    
    This table shows the Hurst exponent values for all selected tokens using 10-minute bars. Every 10-minute interval is displayed, giving you the complete picture of market regime changes throughout the day.
    
    Each row represents a 10-minute time period, and each column represents a different token.
    ...
    **Technical details:**
    - Each Hurst value is calculated using a rolling window of 60 bars (10 hours of 10-minute data)
